...
public class DamqSndProducer {
	...
	public void PushToSendQueue(ModuleType dest, MsgType msgType, String workCode, String msgBody) {
		try {
			...
			// json.org 의 specification 에 "An object is an unordered set of name/value
			// pairs" 라고 나와있다.
			// Human readable 로 만들어주려면 눈으로 읽기 편하게 만들어줘야지.
			// 이 개새끼들은 지네들 편하려고 개판으로 만들어놓았다.
			String buf = String.format(
					"{\"org\":\"%s\",\"dst\":\"%s\",\"date\":\"%s\",\"msgid\":\"%s\",\"msgtype\":\"%s\",\"workcode\":\"%s\",\"body\":%s}",
	...
	private boolean CheckHash(String buf) {
		try {
			...
			String encText = new String(Base64.getEncoder().encodeToString(cipher.doFinal(stamp.getBytes("UTF-8"))))
					.substring(2, 11);
			// 종료 메시지 조건
			// 3분 이내로 발급된 메시지
			// 스탬프의 암호화 값 일치
			if (encText.equals(value) && (Instant.now().toEpochMilli() - Long.parseLong(stamp)) < 180000)
				return true;
		} catch (Exception e) {
			logger.error(e.getMessage());
		}
	...
	public boolean IsTerminateSignal(String buf) {
		try {
			...
			if (jo.get("workcode").toString().equals("terminate")
					...
					&& jo.get("msgtype").toString().equals("req") && CheckHash(jo.get("body").toString())) {
				return true;
			}
		} catch (Exception e) {
			logger.error(e.getMessage());
	...
	public void SendExitSignal() {
		...
		try {
			...
			joBody.put("value", new String(Base64.getEncoder().encodeToString(cipher.doFinal(ooo.getBytes("UTF-8"))))
					.substring(2, 11));
			jo.put("body", joBody);
